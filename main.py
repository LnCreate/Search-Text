...
class SearchApp(QDialog):
    def __init__(self):
        ...
        self.showcontentButton.clicked.connect(self.show_content)
    ...
    def choose_folder(self):
        ...
        def show_result(found_files):
            ...
            for file in found_files:
                self.result_list.addItem(str(file[0])+"  包含关键词  "+ ' , '.join(file[1]))
    ...
    def show_content(self):
        ...
        def show_text(path,keywords):
            ...
            for line_num, line in lines_with_keywords:
                self.textBrowser.append(f"{line_num}: {line}")

        # try:
        # 获取用户选中的结果索引
        selected_index = self.result_list.currentRow()
        if selected_index<0:
            return
        ...
        # 否则，直接显示文本内容
        if self.checkBox.isChecked() and selected_path.endswith('.pdf'):
            highlight_pdf(selected_path, selected_keywords)
        # elif self.checkBox.isChecked() and selected_path.endswith('.docx'):
